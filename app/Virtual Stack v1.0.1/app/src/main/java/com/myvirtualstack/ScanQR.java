            ...
            public void receiveDetections(Detector.Detections<Barcode> detections) {
                ...
                if (barcodes.size() != 0) {
                    txtBarcodeValue.post(new Runnable() {
                        ...
                        public void run() {
                            try{
                                btnAction.setText("CREATE CARD");
                                intentData = barcodes.valueAt(0).displayValue;
//                            txtBarcodeValue.setText(intentData);
                                Intent i = new Intent(ScanQR.this, ViewIndividualContact.class);
                                Bundle b = new Bundle();

                                API api = new API();
                                String json1 = api.createJSON(intentData);
                                ...
                                b.putString("OCCUPATION", occupation);
                    ...
                    new Thread(new Runnable(){
                        ...
                        public void run() {
                            try {
                                API api = new API();
                                api.addCard(user, pass, intentData);
                                System.out.println(api.list(user, pass));
                            }
                            catch (Exception ex) {
                                ex.printStackTrace();
                            }
